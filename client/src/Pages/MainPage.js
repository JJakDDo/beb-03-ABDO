...
const MainPage = () => {
  ...
  function plzAddLike(writingsId) {
    if (userState.authorizedToken !== "") {
      ServerRequestManager.addLikeToWriting(
        userState.authorizedToken,
        writingsId
      )
        .then((res) => {
      ...
      <WritingCardWrapper>
        ...
        {writings.length > 0 ? (
          writings[writingsIndex] ? (
            <WritingCard
              plzAddLike={plzAddLike}
              writingsId={writings[writingsIndex].id}
              nickName={writings[writingsIndex].nickname}
              userId={writings[writingsIndex].writer}
              topic={writings[writingsIndex].title}
              text={writings[writingsIndex].content}
              likes={writings[writingsIndex].likes || "zero"}
              key={`${writings[writingsIndex].writer}_${writings[writingsIndex].title}_${writings[writingsIndex].id}`}
            />
          ) : (
          ...
          <WritingCard
            nickName={dm_writingData.nickName}
            userId={dm_writingData.userId}
            topic={dm_writingData.topic}
            text={dm_writingData.text}
          />
        )}
        <CommentWrapper>
          ...
          {writings ? (
            writings[writingsIndex] ? (
              <CommentCardBox
                commentArray={writings[writingsIndex].comments || []}
              ></CommentCardBox>
            ) : (
